function [K] =  gcp_dist_sign_ef_prox(fileID,client,nd,K,prox,isLogit,isCyclic,params)
...
%                     client(k).U{rd_n} = client(k).U{rd_n} - gamma * client(k).Delta_rd_n;
                    % client(k).U{rd_n} = sign(client(k).U{rd_n} - gamma*client(k).Delta_rd_n) .* max(zeros(size(client(k).U{rd_n})), abs(client(k).U{rd_n} - gamma*client(k).Delta_rd_n)-prox);
                ...
                for k = 1:K
                    % client(k).U{rd_n} = sign(client(k).U{rd_n} - gamma*sumDelta) .* max(zeros(size(client(k).U{rd_n})), abs(client(k).U{rd_n} - gamma*sumDelta)-prox);
